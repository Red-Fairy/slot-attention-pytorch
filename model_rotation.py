...
class WeightedPCA(nn.Module):
    ...
    def forward(self, weight, x):
        ...
        cov = torch.sum(weight * x.unsqueeze(-1) * x.unsqueeze(-2), dim=2) # (b, n, 2, 2)
        ...
        det = eigvec[:, :, 0, 0] * eigvec[:, :, 1, 1] - eigvec[:, :, 0, 1] * eigvec[:, :, 1, 0] # det is 1 or -1
        # if determinant is negative, flip the second column
        # flip the second column
        eigvec[:, :, 0, 1] = eigvec[:, :, 0, 1] * det
...
class EncoderPosEmbedding(nn.Module):
    ...
    def apply_rel_position_scale_rotation(self, grid, position, scale, rotation):
        ...
        inverse_rotation = torch.inverse(rotation).unsqueeze(-2).unsqueeze(-2).expand(b, n, h, w, 2, 2)
        ps = ((grid - position) / (scale * self.scale_factor + 1e-8)) # (b, n, h, w, 2)
        return torch.matmul(inverse_rotation.view(-1, 2, 2), ps.view(-1, 2, 1)).view(b, n, h, w, 2)
...
class DecoderPosEmbedding(nn.Module):
    ...
    def apply_rel_position_scale_rotation(self, grid, position, scale, rotation):
        ...
        inverse_rotation = torch.inverse(rotation).unsqueeze(-2).unsqueeze(-2).expand(bns, h, w, 2, 2)
        ps = ((grid - position) / (scale * self.scale_factor + 1e-8)) # (bns, h, w, 2)
        return torch.matmul(inverse_rotation.view(-1, 2, 2), ps.view(-1, 2, 1)).view(bns, h, w, 2)
