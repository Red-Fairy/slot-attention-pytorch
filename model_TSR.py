...
class WeightedPCA(nn.Module):
    ...
    def forward(self, weight, x):
        ...
        """
        b, n, hw, _ = x.shape
        weight = weight.unsqueeze(-1).expand(b, n, hw, 2)
        # compute weighted mean
        mean = torch.sum(weight * x, dim=2) # (b, n, 2)
        # compute weighted covariance
        x = x - mean.unsqueeze(2).expand(b, n, hw, 2)
        cov = torch.sum(((weight * x).view(-1, 2, 1) * x.view(-1, 1, 2)).view(b, n, hw, 2, 2), dim=2).view(-1, 2, 2) # (b*n, 2, 2)
        # compute eigenvalues and eigenvectors
        eigval, eigvec = torch.linalg.eigh(cov) # (b*n, 2), (b*n, 2, 2)
        # make sure the rotation matrix is left-handed
        # compute determinant of rotation matrix
        with torch.no_grad():
            det = eigvec[:, 0, 0] * eigvec[:, 1, 1] - eigvec[:, 0, 1] * eigvec[:, 1, 0] # det is 1 or -1, (b*n,)
            modify_map = torch.ones(b*n, 2, 2).to(device)
            modify_map[:, 0, 1] = modify_map[:, 0, 1] * det
            modify_map[:, 1, 1] = modify_map[:, 1, 1] * det
        # if determinant is negative, flip the second column
        # flip the second column
        eigvec = eigvec * modify_map # (b*n, 2, 2)
        eigvec = eigvec.view(b, n, 2, 2) # (b, n, 2, 2)
...
def get_rotation_matrix(angle):
    ...
    """
    b, n, _ = angle.shape
    # angle = angle.expand(b, n, 2)
    cos = torch.cos(angle) # (b, n, 1)
    sin = torch.sin(angle)
    return torch.stack([cos, -sin, sin, cos], dim=-1).view(b, n, 2, 2)
...
class EncoderPosEmbedding(nn.Module):
    ...
    def apply_rel_position_scale_rotation(self, grid, position, scale, rotation):
        """
        grid: (1, h, w, 2)
        ...
        inverse_rotation = torch.inverse(rotation.view(-1, 2, 2)).view(b, n, 1, 1, 2, 2).expand(b, n, h, w, 2, 2)
        # inverse_rotation = torch.inverse(rotation).unsqueeze(-3).unsqueeze(-3).expand(b, n, h, w, 2, 2)
        ps = ((grid - position) / (scale * self.scale_factor + 1e-8)) # (b, n, h, w, 2)
        # print(ps.shape, inverse_rotation.shape)
        return torch.matmul(inverse_rotation.reshape(-1, 2, 2), ps.reshape(-1, 2, 1)).reshape(b, n, h, w, 2)

    def forward(self, x, position_latent=None, scale_latent=None, rotation_latent=None):

        k, v = self.input_to_k(x), self.input_to_v(x) # (b, h*w, d)
        k, v = k.unsqueeze(1), v.unsqueeze(1) # (b, 1, h*w, d)

        if position_latent is not None or scale_latent is not None or rotation_latent is not None:
            rel_grid = self.apply_rel_position_scale_rotation(self.grid, position_latent, scale_latent, rotation_latent)
        else:
            rel_grid = self.grid
...
class DecoderPosEmbedding(nn.Module):
    ...
    def apply_rel_position_scale_rotation(self, grid, position, scale, rotation):
        ...
        inverse_rotation = torch.inverse(rotation.view(-1, 2, 2)).view(bns, 1, 1, 2, 2).expand(bns, h, w, 2, 2)
        ps = ((grid - position) / (scale * self.scale_factor + 1e-8)) # (bns, h, w, 2)
        return torch.matmul(inverse_rotation.reshape(-1, 2, 2), ps.reshape(-1, 2, 1)).reshape(bns, h, w, 2)

    def forward(self, x, position_latent, scale_latent, rotation_latent):
        '''
        x: (b*n_s, h, w, d)
        position_latent: (b, n_s, 2)
        '''
        rel_grid = self.apply_rel_position_scale_rotation(self.grid, position_latent, scale_latent, rotation_latent) # (bns, h, w, 2)
...
class SlotAttention(nn.Module):
    def __init__(self, num_slots, dim, iters = 3, eps = 1e-8, hidden_dim = 128, resolution=(128,128)):
        ...
        self.rotation_latent = nn.Parameter(torch.tanh(torch.randn(1, num_slots, 1) * 0.1) * np.pi / 4)
    ...
    def forward(self, inputs, num_slots = None):
        ...
        rotation_latent = get_rotation_matrix(self.rotation_latent.repeat(b, 1, 1))
        ...
        for it in range(self.iters):
            ...
            k, v = self.to_kv(inputs, position_latent, scale_latent, rotation_latent)
            ...
            rel_pos = torch.empty(b, n_s, hw, 2, device=inputs.device) # (b, n_s, h*w, 2)
            for i in range(n_s):
            ...
            rotation_latent = WeightedPCA()(attn, rel_pos)
        ...
        return slots, position_latent, scale_latent, rotation_latent
...
class Decoder(nn.Module):
    ...
    def forward(self, x, position_latent, scale_latent, rotation_latent):
        x = self.decoder_pos(x, position_latent, scale_latent, rotation_latent)
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, num_iterations, hid_dim, decoder_resolution=(16,16), rotation=False):
    ...
    def combine(self, recons, masks):
        masks = nn.Softmax(dim=1)(masks)
        recon_combined = torch.sum(recons * masks, dim=1)  # Recombine image.
        recon_combined = recon_combined.permute(0,3,1,2)
        return recon_combined

    def decode(self, slots, position_latent, scale_latent, rotation_latent, batch_size=1):
        x = self.decoder_cnn(slots, position_latent, scale_latent, rotation_latent)
        recons, masks = x.reshape(batch_size, -1, x.shape[1], x.shape[2], x.shape[3]).split([3,1], dim=-1)
        return recons, masks

    def forward(self, image, return_latents=False):
        ...
        slots, position_latent, scale_latent, rotation_latent = self.slot_attention(x)
        ...
        rotation_latent = rotation_latent.reshape((-1, rotation_latent.shape[-1]))
        ...
        if return_latents:
            return recon_combined, recons, masks, slots, position_latent, scale_latent, rotation_latent
        else:
