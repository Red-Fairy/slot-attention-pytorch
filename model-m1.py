...
class MLPwithPosEmbedding(nn.Module):
    def __init__(self, dim, resolution, hidden_dim=128):
        super().__init__()
        self.grid = build_grid(resolution).view(-1, 4)
        self.grid_embed = nn.Linear(4, dim, bias=True)
        self.input_fc = nn.Linear(dim, dim, bias=False)
...
def build_grid(resolution):
    ...
    return torch.from_numpy(np.concatenate([grid, 1.0 - grid], axis=-1)).to(device) # (h, w, 4)
